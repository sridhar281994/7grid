...
async def _broadcast_match_ready(match: GameMatch, p1: User, p2: User):
    """Notify both players that match is ready via Redis channel"""
    try:
        ...
        print(f"[WARN] Redis publish failed on match:{match.id}: {e}")
...
async def create_or_wait_match(
    ...
    """
    Ensures only ONE waiting match per stake. If found, join it.
    If none, create new. Prevents duplicate matches.
    """
    try:
        ...
        waiting = (
            ...
            .with_for_update(of=GameMatch, nowait=False, skip_locked=True)
            .first()
        )
        ...
        # If no waiting match, create one
        new_match = GameMatch(
...
async def roll_dice(
    ...
    try:
        ...
        print(f"[WARN] Redis publish failed on match:{m.id}: {e}")
