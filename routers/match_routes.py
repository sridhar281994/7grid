...
from routers.wallet_utils import deduct_entry_fee, finalize_payout, refund_entry
...
def _apply_roll(positions: list[int], current_turn: int, roll: int):
    """Apply dice roll with exact win condition + danger zone"""
    p = current_turn
    old = positions[p]
    new_pos = old + roll
    winner = None
...
async def _auto_advance_if_needed(m: GameMatch, db: Session, timeout_secs: int = 10):
    ...
    if winner is not None:
        ...
        await finalize_payout(m, db, winner)
...
async def create_or_wait_match(
    ...
    try:
        ...
        if waiting:
            await deduct_entry_fee(current_user, stake_amount, db)
        ...
        return {
            "ok": True, "match_id": new_match.id, "status": _status_value(new_match),
            "stake": new_match.stake_amount,
            "p1": _name_for(db.get(User, new_match.p1_user_id)),
            "p2": None, "last_roll": new_match.last_roll, "turn": new_match.current_turn,
        }
...
async def check_match_ready(
    ...
    return {
        "ready": m.status == MatchStatus.ACTIVE and m.p1_user_id and m.p2_user_id,
        "finished": m.status == MatchStatus.FINISHED,
        "match_id": m.id, "status": _status_value(m), "stake": m.stake_amount,
...
async def roll_dice(payload: RollIn, db: Session = Depends(get_db),
    ...
    if m.status != MatchStatus.ACTIVE: raise HTTPException(400, "Match not active")
    ...
    try:
        ...
        db.rollback(); raise HTTPException(500, f"DB Error: {e}")
    ...
    return {"ok": True, "match_id": m.id, "roll": roll, "turn": m.current_turn,
            "positions": positions, "winner": winner}

# -------------------------
# Forfeit
# -------------------------
@router.post("/forfeit")
async def forfeit_match(payload: ForfeitIn, db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)) -> Dict:
    ...
    if current_user.id == m.p1_user_id: winner = 1
    elif current_user.id == m.p2_user_id: winner = 0
    else: raise HTTPException(403, "Not your match")
    ...
    try:
        ...
        db.rollback(); raise HTTPException(500, "DB Error")

    await _write_state(m, {"positions":[0,0], "current_turn":m.current_turn,
                           "last_roll":m.last_roll, "winner":winner})
...
async def match_ws(websocket: WebSocket, match_id: int,
    ...
    r = await _get_redis(); pubsub = None
    if r:
        try:
            pubsub = r.pubsub()
            ...
            print(f"[WS] Redis pubsub subscribe error: {e}"); pubsub = None
    ...
    try:
        while True:
            ...
            if pubsub:
                try:
                    ...
                    if msg and msg.get("type")=="message":
                        await websocket.send_text(msg["data"]); sent = True
                except Exception as e:
            ...
            if not sent:
                ...
                try:
                    m = db.query(GameMatch).filter(GameMatch.id==match_id).first()
                    if not m:
                        await websocket.send_text(json.dumps({"error":"Match not found"})); break
                    if m.status == MatchStatus.ACTIVE:
                        await _auto_advance_if_needed(m, db)

                    st = await _read_state(match_id) or {
                        "positions":[0,0], "current_turn":m.current_turn or 0,
                        "last_roll":m.last_roll, "winner":None,
                    }
                    snapshot = {
                        "ready": m.status == MatchStatus.ACTIVE and m.p1_user_id and m.p2_user_id,
                        "finished": m.status == MatchStatus.FINISHED,
                        "match_id": m.id, "status": _status_value(m),
                        "stake": m.stake_amount,
                        "p1": _name_for(db.get(User, m.p1_user_id)) if m.p1_user_id else None,
                        "p2": _name_for(db.get(User, m.p2_user_id)) if m.p2_user_id else None,
        ...
        if pubsub:
            try:
                await pubsub.unsubscribe(f"match:{match_id}:events"); await pubsub.close()
            except Exception:
                pass
