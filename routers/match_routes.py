...
def list_waiting_matches(
    ...
    # ignore invalid rows (where p1 is None)
    q = q.filter(GameMatch.p1_user_id.isnot(None))
...
def create_or_wait_match(
    ...
    """
    Try to join an existing waiting match with same stake.
    If none available, create new one with this user as p1.
    """
    # 1) Try to join existing waiting match (not your own)
    existing = (
        db.query(GameMatch)
        .filter(
    ...
    # 2) Create new match with this user as Player 1
    new_match = GameMatch(
...
def join_match(
    ...
    if m.status != MatchStatus.WAITING or not m.p1_user_id:
        raise HTTPException(400, "Match is not available for joining")
    ...
    db.commit()
    db.refresh(m)
    return {"ok": True, "joined": True, "match": _match_to_dict(m, db)}
