...
def recharge_initiate(payload: AmountIn, db: Session = Depends(get_db),
    ...
    """
    Create a pending recharge. In production, redirect to UPI gateway.
    For now, returns a fake reference to be confirmed by /recharge/mock-success
    or by real gateway webhook hitting /recharge/callback.
    """
    ...
    db.commit()
    db.refresh(tx)
    return {"ok": True, "tx_id": tx.id, "hint": "Call /wallet/recharge/mock-success?tx_id=... in dev"}
...
def recharge_mock_success(tx_id: int, db: Session = Depends(get_db)):
    ...
    if not tx or tx.type != TxType.RECHARGE:
...
def withdraw(payload: WithdrawIn, db: Session = Depends(get_db),
    ...
    # Dev mode: auto-success if MOCK_PAYOUT=true
